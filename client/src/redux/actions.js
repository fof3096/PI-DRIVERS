...
export const getDrivers = () => {
  return async (dispatch) => {
    try {
      ...
      return dispatch({
        // Sí no me equivoco ☝, el dispatch es una propiedad que viene por default en la store
        type: GET_DRIVERS,
...
export const filterByTeam = (teamSelected, drivers) => {
  return async (dispatch) => {
    try {
      const toPayload = drivers.filter((driver) =>
        driver.teams.some((team) => team == teamSelected)
      );
      return dispatch({
        type: FILTER_BY_TEAM,
        payload: toPayload,
...
export const deleteDriver = (idDriver) => {
  return async () => {
    try {
      await axios.delete("http://localhost:3001/drivers", {
        data: {
          id: idDriver,
        },
      });
