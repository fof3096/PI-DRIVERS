  ...
  if (name) {
    ...
    try {
      ...
      response.data.map((driver) => {
        if (
          driver.name?.forename.startsWith(name) ||
          driver.name?.surname.startsWith(name)
        ) {
          let teamsArray = [];
          if (driver.teams) {
...
const postDrivers = async (req, res) => {
  let { forename, surname, description, image, nationality, birthDate, teams } =
    req.body;
...
const deleteDrivers = async (req, res) => {
  ...
  try {
    ...
    res.send("Corredor eliminado");
  } catch (error) {
    res.status(400).json({ error: "Error al eliminar el Corredor" });
  }
};
